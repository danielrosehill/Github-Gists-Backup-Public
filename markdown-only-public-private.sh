#!/bin/bash
# Script to pull GitHub gists with anonymized credentials
# Splits output between public and private gists while extracting markdown files

# Configuration
GITHUB_USERNAME="your_github_username"
# Path for gist backups
BACKUP_PATH="./github-gists-backup"
# Path for extracted markdown files
MARKDOWN_PATH="./github-gists-markdown"
GITHUB_TOKEN="your_github_token"
# Max length for folder name prefix (from file name)
MAX_NAME_LENGTH=30
# Current date in DDMMYY format for folder naming
CURRENT_DATE=$(date '+%d%m%y')
# Directories for flat markdown files structure, separated by visibility
MARKDOWN_PUBLIC_DIR="$MARKDOWN_PATH/public"
MARKDOWN_PRIVATE_DIR="$MARKDOWN_PATH/private"
...
extract_markdown_files() {
  ...
  # Determine if this is a public or private gist based on the folder path
  local visibility="private"
  if [[ "$gist_folder" == *"/public/"* ]]; then
    visibility="public"
  fi
  
  # Set the appropriate markdown directory based on visibility
  local markdown_dir
  if [ "$visibility" = "public" ]; then
    markdown_dir="$MARKDOWN_PUBLIC_DIR"
  else
    markdown_dir="$MARKDOWN_PRIVATE_DIR"
  fi
  ...
  find "$gist_folder" -type f -name "*.md" | while read -r md_file; do
    ...
    if [ -f "$target_file" ]; then
      log "File $base_filename already exists in $visibility markdown directory"
      counter=1
    ...
    log "Copied markdown file to $visibility directory: $(basename "$target_file")"
  done
}

# Create backup directory
log "Starting GitHub gists backup to $BACKUP_PATH"
log "Markdown files will be extracted to $MARKDOWN_PATH"
mkdir -p "$BACKUP_PATH/public" "$BACKUP_PATH/private" "$MARKDOWN_PUBLIC_DIR" "$MARKDOWN_PRIVATE_DIR" || {
...
log "- Markdown Path: $MARKDOWN_PATH"
log "- GitHub Token: ${GITHUB_TOKEN:0:4}... (truncated)"
...
log "Markdown files location: $MARKDOWN_PATH"
...
PUBLIC_MD_COUNT=$(find "$MARKDOWN_PUBLIC_DIR" -type f -name "*.md" | wc -l)
PRIVATE_MD_COUNT=$(find "$MARKDOWN_PRIVATE_DIR" -type f -name "*.md" | wc -l)
TOTAL_MD_COUNT=$((PUBLIC_MD_COUNT + PRIVATE_MD_COUNT))
log "Total markdown files extracted: $TOTAL_MD_COUNT (Public: $PUBLIC_MD_COUNT, Private: $PRIVATE_MD_COUNT)"
